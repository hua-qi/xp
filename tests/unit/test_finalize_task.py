...
async def test_finalize_task_extract_fails_session_still_runs():
    ...
    extract_handler = ExtractHandler(uow_factory=InMemoryUnitOfWork)
    session_handler = SessionHandler(uow_factory=InMemoryUnitOfWork)

    result = {"extract": {"status": "skipped", "id": None, "reason": None}, "session": {"status": "skipped"}}

    try:
        exp = await extract_handler.handle_extract(ExtractExperienceCommand(
    ...
    try:
        await session_handler.handle_record_session(RecordSessionCommand(
...
async def test_finalize_task_all_steps_succeed(monkeypatch):
    from src.embeddings import EmbeddingProvider, set_provider
    from src.application.handlers.extract_handler import ExtractHandler
    from src.application.handlers.session_handler import SessionHandler
    from src.application.commands import ExtractExperienceCommand, RecordSessionCommand
...
def test_server_finalize_task_extract_fails_session_recorded():
